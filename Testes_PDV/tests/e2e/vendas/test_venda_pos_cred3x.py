"""Test POS Crédito 3x - Venda consumidor com POS crédito 3x (nome descoberto dinamicamente)."""
import pytest
import allure
...
class TestVendaPosCred3x:

    @allure.title("Venda Consumidor - POS Crédito 3x (descoberta dinâmica)")
    @allure.severity(allure.severity_level.CRITICAL)
    @allure.tag("venda", "consumidor", "pos", "credito", "3x")
    def test_venda_consumidor_pos_credito_3x(self, driver_logado):
        """Venda consumidor com POS crédito parcela 0+3 — forma descoberta dinamicamente.
        ...
        with allure.step("6. Selecionar POS Crédito 3x (qualquer base)"):
            parcela = test_data.PARCELAS_CREDITO[2] if len(test_data.PARCELAS_CREDITO) > 2 else "A Prazo 0 + 3"
        ...
        with allure.step("9. Verificar retorno à tela inicial"):
            assert home.tela_inicial_exibida(), "Não voltou para tela inicial após venda POS crédito 3x"

        logger.info(f"[POS CRÉDITO 3x] Venda consumidor {nome} 0+3 OK")
