"""Test POS Crédito 2x - Venda consumidor com POS crédito 2x (nome descoberto dinamicamente)."""
import pytest
import allure
...
class TestVendaPosCred2x:

    @allure.title("Venda Consumidor - POS Crédito 2x (descoberta dinâmica)")
    @allure.severity(allure.severity_level.CRITICAL)
    @allure.tag("venda", "consumidor", "pos", "credito", "2x")
    def test_venda_consumidor_pos_credito_2x(self, driver_logado):
        """Venda consumidor com POS crédito parcela 0+2 — forma descoberta dinamicamente.
        ...
        with allure.step("6. Selecionar POS Crédito 2x (qualquer base)"):
            parcela = test_data.PARCELAS_CREDITO[1] if len(test_data.PARCELAS_CREDITO) > 1 else "A Prazo 0 + 2"
        ...
        with allure.step("9. Verificar retorno à tela inicial"):
            assert home.tela_inicial_exibida(), "Não voltou para tela inicial após venda POS crédito 2x"

        logger.info(f"[POS CRÉDITO 2x] Venda consumidor {nome} 0+2 OK")
