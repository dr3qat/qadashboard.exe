...
class TestTrocaPageDefinirDataInicial:
    ...
    def test_definir_data_inicial_com_data_fornecida(self, mock_time, mock_logger, mock_base_init):
        ...
        Usa ver_e_clicar para abrir o campo (scroll automático).
        """
        ...
        # Assert
        page.ver_e_clicar.assert_called_once_with(TrocaPage.INPUT_DATA_INICIAL)
...
class TestTrocaPageClicarConsultar:
    ...
    @patch('pages.troca_page.BasePage.__init__', return_value=None)
    @patch('pages.troca_page.logger')
    def test_clicar_consultar_fecha_teclado_rola_e_aguarda_lista(self, mock_logger, mock_base_init):
        """
        Deve fechar teclado, usar ver_e_clicar no botão consultar
        e aguardar a lista de notas aparecer (wait dinâmico).
        """
        from pages.troca_page import TrocaPage
        ...
        page.ver_e_clicar.assert_called_once_with(TrocaPage.BTN_CONSULTAR)
        page.encontrar_por_id.assert_called_once_with(
            TrocaPage.ITEM_LISTA_NOTAS, tempo_espera=10
        )
...
class TestTrocaPageConfirmarDialogos:
    ...
    def test_confirmar_dialogos_clica_nos_popups(self, mock_time, mock_logger, mock_base_init):
        """
        Deve tentar clicar nos dois popups de confirmação via clicar_se_existir.
        Primeiro com tempo_espera=5, depois com tempo_espera=3.
        """
        from pages.troca_page import TrocaPage
        ...
        # Assert: dois popups tentados via clicar_se_existir
        assert page.clicar_se_existir.call_count == 2
        page.clicar_se_existir.assert_any_call(TrocaPage.BTN_DIALOGO_OK, tempo_espera=5)
        page.clicar_se_existir.assert_any_call(TrocaPage.BTN_DIALOGO_OK, tempo_espera=3)
        # clicar_por_id não é chamado diretamente em confirmar_dialogos
...
class TestTrocaPageSelecionarPagamentoBonus:
    ...
    def test_selecionar_pagamento_bonus_ativa_switch_e_avanca(self, mock_time, mock_logger, mock_base_init):
        ...
        # Assert
        page.elemento_existe.assert_called_once_with(TrocaPage.SWITCH_BONUS, tempo_espera=5)
...
class TestTrocaPageValidacoes:
    ...
    def test_validar_e_fechar_sucesso(self, mock_logger, mock_base_init):
        """
        Deve aguardar 'Venda realizada com sucesso!' e chamar concluir_venda.
        """
        from pages.troca_page import TrocaPage
...
class TestTrocaPageFluxoTroca:
    ...
    @patch('pages.troca_page.logger')
    @patch('pages.troca_page.time')
    def test_executar_troca_chama_metodos_na_ordem(self, mock_time, mock_logger, mock_base_init):
        """
        Deve chamar todos os métodos do fluxo na ordem correta,
        incluindo venda pós-troca com bônus.
        """
        from pages.troca_page import TrocaPage
        ...
        page.validar_sucesso_troca = MagicMock()
        page.voltar_com_confirmacao = MagicMock()
        ...
        page.validar_sucesso_troca.assert_called_once()
        page.voltar_com_confirmacao.assert_called_once()

    @patch('pages.troca_page.BasePage.__init__', return_value=None)
    @patch('pages.troca_page.logger')
    @patch('pages.troca_page.time')
    def test_executar_troca_usa_data_padrao(self, mock_time, mock_logger, mock_base_init):
        """
...
class TestTrocaPageFluxoTrocaConsumidor:
    ...
    def test_executar_troca_consumidor_fluxo_completo(self, mock_test_data, mock_time, mock_logger, mock_base_init):
        """
        Deve executar fluxo completo de troca consumidor com venda pós-troca via bônus.
        """
        from pages.troca_page import TrocaPage
        ...
        # aguardar_texto e ver_e_clicar são chamados diretamente ao final do fluxo
        page.aguardar_texto = MagicMock()
        page.ver_e_clicar = MagicMock()
        ...
        # Act
        page.executar_troca_consumidor(data)
        ...
        page.aguardar_texto.assert_called_once_with("Venda realizada com sucesso!", tempo_espera=30)
        page.ver_e_clicar.assert_called_once_with(TrocaPage.BTN_CONFIRMAR_VENDA)

    @patch('pages.troca_page.BasePage.__init__', return_value=None)
    @patch('pages.troca_page.logger')
    @patch('pages.troca_page.time')
    @patch('pages.troca_page.test_data')
    def test_executar_troca_consumidor_seleciona_cliente_quando_popup_sim(self, mock_test_data, mock_time, mock_logger, mock_base_init):
        """
        ...
        # Arrange
        mock_test_data.CUSTOMER_ID_TROCA = "42"
        page = TrocaPage.__new__(TrocaPage)
        page.driver = MagicMock()
        page.definir_data_inicial = MagicMock()
        page.clicar_consultar = MagicMock()
